...
Networking.prototype.send = function (packet, directAddr) {
  ...
  function onEach(connection) {
    ...
    else if (directAddr && sock.address().port === 5353) {
      debug('skip direct send from non-fallback port.');
    }
    else {
      debug('sending to', sock.address());
      sock.send(buf, 0, buf.length, 5353, directAddr || '224.0.0.251', function (err, bytes) {
